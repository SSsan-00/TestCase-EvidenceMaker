...
- 通常の文字列・文字リテラル内、および行コメント（`//`、互換対応の `#`）・ブロックコメント（`/* ... */`）内の記述は対象外です。
...
- 通常補間文字列（`$"..."`）と Verbatim 補間文字列（`$@"..."` / `@$"..."`）では、単独の `{...}` 内だけを C# コードとして検索します。
- `{{` / `}}` は文字列へ波括弧を出力する記述として扱い、その内側に見える関数名は検索しません。
- raw 補間文字列（`$"""..."""`）に対応します。開始時の引用符が4個以上の場合も、同数の引用符までを1つの文字列として扱います。
- raw 補間文字列の先頭に `$` が N個ある場合、N個の `{` と `}` で囲まれた部分だけを補間式として検索します。例えば `$$"""{{sqlS(XXX)}}"""` は装飾し、`$$"""{sqlS(XXX)}"""` は装飾しません。
- 補間式内に通常文字列、コメント、別の補間文字列、オブジェクト初期化子がある場合も、字句状態と波括弧の深さを維持して解析します。
- 補間式の `:` より後ろにあるformat指定は文字列として扱い、エスケープ関数の検索対象にしません。
- Verbatim/raw補間文字列と補間式が B列の複数行に跨る場合も、文字列の終了位置と補間式の終了位置まで継続して解析します。
